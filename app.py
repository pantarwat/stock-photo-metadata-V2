...
# --- 1. Config ---
st.set_page_config(page_title="AI Stock Vision - Server Protected", layout="wide")
st.title("🎯 AI Stock Vision (Server Space Protected)")

# --- 2. Sidebar ---
with st.sidebar:
    st.header("⚙️ Settings")
    
    # 🚨 ปุ่มสำคัญที่สุด: ล้างแคชและคืนพื้นที่ให้เซิร์ฟเวอร์ทันที
    if st.button("🗑️ ล้างข้อมูล/เริ่มใหม่ (เคลียร์ไฟล์สะสมบนเซิร์ฟเวอร์)", type="primary", use_container_width=True):
        st.session_state.clear() # ล้าง RAM แคชทั้งหมด
        st.rerun()
    ...
    if selected_model_display == "กรอกชื่อโมเดลเอง (Custom)":
        model_choice = st.text_input("✏️ พิมพ์รหัส API โมเดล", value="gpt-5")
    else:
...
# 🚨 ระบบลบไฟล์ขยะอัตโนมัติบน Harddrive ของเซิฟเวอร์
def process_to_jpg_iptc(uploaded_file_bytes, title, keywords):
    ...
    try:
        ...
        # สั่งลบไฟล์ทิ้งทันทีหลังประมวลผลเสร็จ เพื่อไม่ให้พื้นที่เต็ม
        if tmp_path:
            try:
                os.unlink(tmp_path)
