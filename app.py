...
# Import Streamlit as early as possible so startup/import errors can be shown
...
try:
    ...
    st.set_page_config(page_title="AI Stock Vision — Startup Error", page_icon="⚠️", layout="wide")
    st.error("เปิดแอปได้ แต่โหลด Pillow ไม่สำเร็จ")
    st.code(f"{type(exc).__name__}: {exc}\n\n{traceback.format_exc()}", language="text")
    st.info("ตรวจ requirements.txt และ Python version ของ Streamlit Community Cloud")
    st.stop()
...
try:
    ...
    st.set_page_config(page_title="AI Stock Vision — Startup Error", page_icon="⚠️", layout="wide")
    st.error("เปิดแอปได้ แต่โหลด OpenAI/Pydantic ไม่สำเร็จ")
    st.code(f"{type(exc).__name__}: {exc}\n\n{traceback.format_exc()}", language="text")
    st.info("ตรวจ requirements.txt และ Python version ของ Streamlit Community Cloud")
    st.stop()
